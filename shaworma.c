...
int ingredient_no = 0;
// VARIABILA IN CARE TINEM MARIMEA ACTIVA COSULUI
int basket_length = 0;
...
void render_custom_shaworma_menu()
{
  ...
  int sscanf_result;
  answer = -1;
  // CAT TIMP RASPUNSUL NU ESTE 13 - FINALIZAREA CONFIGURARII SAU INAPOI LA MENIU
  while (answer != 13 && answer != 14)
  {
    printf("\n=== MENIU INGREDIENTE ===\n");
    ...
    // DACA UTILIZATORUL A ADAUGAT CEL PUTIN DOUA INGREDIENTE, POATE TERMINA SHAWORMA SI O POATE ADAUGA IN COS
    if (ingredient_no > 1)
    {
      printf("[13] Atat boss\n");
    }
    printf("[14] Inapoi la meniu\n");
